...
final class CreateExpenseBloc
  ...
  FutureOr<void> _onStarted(
    ...
    final currentUserId = (event.params.currentUserId?.isNotEmpty == true)
        ? event.params.currentUserId!
        : (cachedUser?.id ?? '');
    final currentUserEmail = cachedUser?.email?.toLowerCase();
      ...
      if (fId != null) {
        final isMe =
            ...
            (currentUserEmail != null &&
                friend.email?.toLowerCase() == currentUserEmail);
        if (isMe) {
          userNames[fId] = 'You';
        } else if (!userNames.containsKey(fId)) {
    ...
    for (final group in groups) {
      for (final member in group.members) {
        ...
        if (mId != null) {
          ...
          if (isMe) {
            userNames[mId] = 'You';
          } else if (!userNames.containsKey(mId)) {
    ...
    if (expense != null) {
      ...
      final alreadyHasMe = participants.any(
        (id) => id == currentUserId || userNames[id] == 'You',
      );
      if (currentUserId.isNotEmpty && !alreadyHasMe) {
        participants.add(currentUserId);
      }

      final meId = participants
          .where((id) => id == currentUserId || userNames[id] == 'You')
          .firstOrNull;
      final paidBy =
          meId ??
  ...
  void _onGroupSelected(
    ...
    if (groupId != null && groupId.isNotEmpty) {
      ...
      final meId = memberIds
          .where((id) => state.store.userNames[id] == 'You')
          .firstOrNull;
      final paidBy = state.store.paidBy.isNotEmpty
          ? (memberIds.contains(state.store.paidBy)
                ? state.store.paidBy
                : (meId ??
                      (memberIds.isNotEmpty
                          ? memberIds.first
                          : state.store.paidBy)))
          : (meId ?? (memberIds.isNotEmpty ? memberIds.first : ''));
